...
if __name__ == "__main__":
  ...
  for file in input_dir.iterdir():
    fstr = str(file)
    if fstr.find("logsiticreg.csv") != -1:
  ...
  for file in encoding_dir.iterdir():
    fstr = str(file)
  ...
  meta = pd.read_csv(encoding_dir / "taxa_parent_child.txt", sep="\t")
  print(" ", meta.shape)
  ...
  for class_lvl in class_lvl_orders:
    if int(class_lvl) in meta_sub.label.values:
      class_lvl_orders[class_lvl] = list(meta_sub.loc[meta_sub.label==int(class_lvl), "child_labels"].values)[0].split(",")
  ...
  for order_lvl in order_lvl_families:
    if int(order_lvl) in meta_sub.label.values:
      order_lvl_families[order_lvl] = list(meta_sub.loc[meta_sub.label==int(order_lvl), "child_labels"].values)[0].split(",")
  # Iterate through all the photoids in max_class_preds
  for row in range(max_class_preds.shape[0]):
    photoid = max_class_preds.index[row] # photo id
  ...
  max_class_preds.columns = ['max_prob_class', 'class', 'max_prob_order',
    ...
    'genus', 'max_prob_species', 'species', 'joint_prob']
  ...
  print("Saving final file with missing taxa backfilled")
